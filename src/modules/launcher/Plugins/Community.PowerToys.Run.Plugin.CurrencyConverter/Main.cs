...
namespace Community.PowerToys.Run.Plugin.CurrencyConverter
{
    public class Main : IPlugin, IPluginI18n, IContextMenu, IDisposable
    {
        ...
        public List<Result> Query(Query query)
        {
            ...
            // Convert
            return CurrencyHandler.Convert(convertModel)
                .Select(x => GetResult(x))
                .ToList();
        }

        private Result GetResult(CurrencyConversionResult result)
        {
            return new Result
            {
                ContextData = result,
        ...
        private ContextMenuResult CreateContextMenuEntry(CurrencyConversionResult result)
        {
        ...
        {
            if (!(selectedResult?.ContextData is CurrencyConversionResult))
            {
                return new List<ContextMenuResult>();
            }

            List<ContextMenuResult> contextResults = new List<ContextMenuResult>();
            CurrencyConversionResult result = selectedResult.ContextData as CurrencyConversionResult;
